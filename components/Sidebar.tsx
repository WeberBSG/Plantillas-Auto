  ...
  return (
    <div className="p-6">
      ...
      <div className={`mb-8 p-3 rounded-xl border space-y-4 ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-gray-50 border-gray-100'}`}>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className={miniLabelClasses}>TUCUMÁN ADDRESS</label>
            <button 
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className={miniLabelClasses}>DNI GENERATOR</label>
            <button 
      ...
      <div className={`flex items-center justify-between mb-4 border-b pb-2 ${isDark ? 'border-gray-800' : 'border-gray-100'}`}>
        ...
        <span className="text-[10px] font-bold text-gray-400 uppercase">{template.elements.length + 1} Layers</span>
      </div>
          ...
          >
            <div className="flex items-center gap-3 overflow-hidden flex-1">
              ...
              <div className="flex flex-col flex-1 overflow-hidden">
                <span className={`text-xs font-extrabold truncate uppercase tracking-wide ${isDark ? 'text-indigo-400' : 'text-indigo-600'}`}>Canvas Background</span>
                {!baseIsExpanded && (
                  <span className="text-[10px] text-gray-500 flex items-center gap-1">
                    System Layer • Opacity:{Math.round((template.baseOpacity ?? 1) * 100)}%
                  </span>
                )}
              </div>
            </div>
            <div className="flex items-center gap-1">
              <Lock className={`w-3.5 h-3.5 ${isDark ? 'text-gray-700' : 'text-gray-300'}`} title="Base layer position is locked" />
            </div>
          </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <label className={`text-[10px] font-bold uppercase ${isDark ? 'text-gray-600' : 'text-gray-500'}`}>Rotation</label>
                ...
                <div className={`grid grid-cols-2 gap-4 border-t pt-3 ${isDark ? 'border-gray-700' : 'border-gray-50'}`}>
                  <div className="space-y-3">
                    <div>
                      <label className={`${subLabelClasses} flex items-center gap-1`}><Layers className="w-2.5 h-2.5" /> Blend Mode</label>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className={subLabelClasses}>Opacity</label>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between mb-1">
                      <label className={`${subLabelClasses} flex items-center gap-1`}><Square className="w-2.5 h-2.5" /> Round Edges</label>
                    ...
                    <div className="flex items-center justify-center p-2 bg-gray-50 dark:bg-gray-900 rounded-lg border border-dashed border-gray-200 dark:border-gray-700">
                      <p className="text-[9px] text-gray-400 text-center uppercase leading-tight font-bold">This layer sets the dimensions of your template</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden flex-1">
                    ...
                    <div className="flex flex-col flex-1 overflow-hidden">
                      <span className={`text-xs font-bold truncate ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{el.name || (isPhoto ? `Photo Layer` : el.content.substring(0, 15))}</span>
                      {!isExpanded && (
                        <span className="text-[10px] text-gray-500 flex items-center gap-1">
                          Z:{el.zIndex} • Opacity:{Math.round((el.opacity ?? 1) * 100)}%
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 animate-in slide-in-from-top-1 duration-200">
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <label className={subLabelClasses}>Layer Name</label>
                        <input 
                          type="text"
                          value={el.name || ''}
                          onChange={(e) => onUpdateElement(el.id, { name: e.target.value })}
                          placeholder="e.g. Center Logo"
                      ...
                      <div>
                        <label className={subLabelClasses}>Hierarchy</label>
                        <div className="flex gap-1">
                          <button onClick={() => onReorderElement(el.id, 'front')} className={`p-1.5 border rounded-lg transition-colors ${isDark ? 'border-gray-700 hover:bg-gray-700 text-indigo-400' : 'border-gray-100 hover:bg-gray-50 text-indigo-600'}`} title="To Front"><ChevronsUp className="w-4 h-4" /></button>
                          <button onClick={() => onReorderElement(el.id, 'back')} className={`p-1.5 border rounded-lg transition-colors ${isDark ? 'border-gray-700 hover:bg-gray-700 text-indigo-400' : 'border-gray-100 hover:bg-gray-50 text-indigo-600'}`} title="To Back"><ChevronsDown className="w-4 h-4" /></button>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className={`grid grid-cols-2 gap-4 border-t pt-3 ${isDark ? 'border-gray-700' : 'border-gray-50'}`}>
                          <div className="space-y-3">
                            <div className="flex items-end gap-2">
                              <div className="flex-1">
                                <label className={subLabelClasses}>Width (px)</label>
                                <input 
                                  type="number" 
                            ...
                            <div>
                              <label className={subLabelClasses}>Height (px)</label>
                              <input 
                                type="number" 
                      ...
                      <div className="space-y-4">
                        ...
                        <div className={`flex items-center justify-between p-2 rounded-lg border ${isDark ? 'bg-gray-900 border-gray-700' : 'bg-gray-50 border-gray-100'}`}>
                          <label className="flex items-center gap-2 cursor-pointer select-none">
                            ...
                            />
                            <span className={`text-[10px] font-bold uppercase ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Enable Random ID</span>
                          </label>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <label className={subLabelClasses}>Font Family</label>
                          ...
                          <div>
                            <label className={subLabelClasses}>Size (px)</label>
                        ...
                        <div className="flex items-center justify-between pt-2">
                          ...
                          <div className="flex items-center gap-1">
                            <label className={`text-[10px] font-bold uppercase mr-1 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>Rotation</label>
                    ...
                    <div className={`space-y-3 p-3 rounded-lg border ${isDark ? 'bg-gray-900/50 border-gray-700' : 'bg-indigo-50/10 border-gray-100'}`}>
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          <label className={`text-[10px] font-bold uppercase ${isDark ? 'text-gray-600' : 'text-gray-500'}`}>X Position (%)</label>
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          <label className={`text-[10px] font-bold uppercase ${isDark ? 'text-gray-600' : 'text-gray-500'}`}>Y Position (%)</label>
      ...
      {template.elements.length === 0 && !template.baseImage && (
        <div className={`mt-10 py-12 text-center border-2 border-dashed rounded-2xl ${isDark ? 'border-gray-800' : 'border-gray-100'}`}>
          ...
          <p className={`text-sm ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>No elements yet.<br/>Use the buttons above to add layers.</p>
        </div>
      )}
    </div>
  );
